...
def mutateExploreTime(chrom): 
    chrom[iExploreTime] = random.randint(1, 20) # for now max = signalTimeMax
    
def mutateSignalTime(chrom):
    chrom[iSignalTime] = random.randint(1, 20)

def mutateSignalCloseRange(chrom): # Usefulness depends on patch size, maybe set limit relative to patch size here?
    chrom[iSignalCloseRange] = random.randint(1, 20)
...
def generateChromosome():
    ...
    c[iExploreTime] = random.randint(1,20) # per 100 # 300
    c[iSignalTime] = random.randint(1,50) # per 1000 # 2000
    c[iSignalCloseRange] = random.randint(6, 15) # per 10 # 50 # should be bigger than obstacle avoidance distance
